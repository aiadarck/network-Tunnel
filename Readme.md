...
مفهوم انجام این نوع تونل بدین شرح است که ما از طریق سرور فیلتر شده به سرور ایران ارتباط SSH برقرار میکنیم، سپس با استفاده از همین ارتباط پایدار به صورت معکوس پورت tcp مورد نظر را از سرور ایران به پورت tcp سرور فیلتر شده هدایت میکنیم (port forwarding)، در نتیجه کاربرات با استفاده از پورت tcp هدایت شده از سرور ایران میتوانند به سرور فیلتر شده دسترسی داشته باشن.
...
<h2 dir="rtl">1. آماده سازی سرور ایران</h2>
<p dir="rtl">توجه: این مرحله روی سرور ایران انجام میشود</p>

فرمان زیر امکان گوش دادن به درخواست های ورودی به سرور ایران را توسط پورت های فوروارد شده از طریق تونل باز میکند:
...
<h2 dir="rtl">2. تهیه کلید SSH درون سرور فیلتر شده و شناخت آن به سرور ایران</h2>
...
حال با فرمان زیر کلید عمومی ساخته شده در سرور ایران کپی میشود:
...
از این رو که اولین باری هست که سرور فیلتر به سرور ایران ارتباط ssh ایجاد میکند از شما درباره اعتماد به فینگرپرینت سرور ایران سوال میشود که باید جواب دهید:

```bash
yes
```

سپس با وارد کردن پسورد سرور ایران، این روند تکمیل میشود که در نتیجه دفعات بعدی شما میتوانید بدون استفاده از پسورد از درون سرور فیلتر به سرور ایران دسترسی داشته باشید

در حال حاضر متوجه شدید که بعد از انجام عملیات کپی کلید، وارد سرور ایران شده و داره بهتون شل سرور ایران رو نشون میده، ما باقی مراحلمونم هنوز رو سرور فیلتر باید انجام بشه پس بنویسید exit و از سرور ایران خارج بشید که برگرده رو سرور فیلتر تا ادامه بدیم.

<p dir="rtl">*نکته، محض اطلاع: این روند به صورت اتوماتیک انجام میشود، مقصد کپی کردن کلید در سرور ایران  فایل ذکر شده میباشد:*
...
<p>در اینجا 0.0.0.0:99 روی سرور ایران تنظیم میشه که تایین میکنه از تمام ای پی های ممکن (0.0.0.0 تنها و تنها به معنی تمام ای پی ها هست، هیچ معنی دیگری ندارد) روی پورت 99 درخواست ها رو قبل کن و هدایت کن به سرور خودمون</p>
...
<h2 dir="rtl">5. تست کردن وضعیت تونل روی سرور ایران</h2>
<p dir="rtl">توجه: این مرحله روی سرور ایران انجام میشود</p>
<p dir="rtl">مهم ترین تست اینه که رو سرور ایران باید بتونید با وارد کردن فرمان زیر:</p>
...
<p dir="rtl">تمام پورت هایی که روی سرور ایران در حال listen شدن هستن رو مشاهده کنید</p>
...
<p dir="rtl">در نهایت هم یادتون باشه همیشه ریبوت کردن سرور ایران میتونه مشکلات قطعی تونل رو حل کنه</p>
